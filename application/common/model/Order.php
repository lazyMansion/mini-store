...
class Order extends BaseModel
{
    ...
    */
    public function path($item_id,$pay_price,$path,$fen=0)
    {
        ...
        if(isset($path)){
            $values=unserialize($setting['values']);

            if(isset($path[0])&& isset($values['first_money'])){
                $agent['first']=$data['agent_price']*$values['first_money']/100;
                if($fen==1 && $agent['first']>0)$this->apple($path[0],1,$agent['first']);
            }
            if(isset($path[1])&& isset($values['second_money'])){
                $agent['second']=$data['agent_price']*$values['second_money']/100;
                if($fen==1 && $agent['second']>0) $this->apple($path[1],2,$agent['second']);
            }
            if(isset($path[2]) && isset($values['third_money'])){
                $agent['third']=$data['agent_price']*$values['third_money']/100;
                if($fen==1 && $agent['third']>0) $this->apple($path[2],3,$agent['third']);
            }
            if(isset($agent))
                return $agent;
        }
    ...
    private function transferDataType($dataType)
    {
        ...
        switch ($dataType) {
            ...
            case 'gold':
                ...
                $values=unserialize($data['values']);
                $filter = ['pay_status' => 20,'delivery_status'=>20,'end_time'=>0,'update_time'=>['>',time()-$values['order']['receive_days']*86400]];
                break;
        }
        return $filter;
    }
